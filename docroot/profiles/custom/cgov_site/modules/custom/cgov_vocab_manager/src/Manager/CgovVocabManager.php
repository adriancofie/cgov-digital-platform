...
class CgovVocabManager {
  ...
  public function getTaxonomyBreadcrumb(int $parent_tid, TermStorageInterface $storageController, RendererInterface $renderer) {
    ...
    while ($parent = array_shift($parents)) {
      ...
      if ($first_element) {
        ...
        $parent_link_url = Url::fromRoute('entity.taxonomy_vocabulary.overview_onelevel_form', [
    ...
    $parents_links[] = Link::fromTextAndUrl('Level 0 (Root)', Url::fromRoute('entity.taxonomy_vocabulary.overview_onelevel_form', [
  ...
  public function getChildrenLink(Term $term, TermStorageInterface $storageController, RendererInterface $renderer) {
    ...
    // Generate the link.
    $link_manage_children_url = Url::fromRoute('entity.taxonomy_vocabulary.overview_onelevel_form', [
      'taxonomy_vocabulary' => $term->getVocabularyId(),
      'parent_tid' => $term->id(),
    ]);
